...
Пороги батареи лежат в `config.yaml`:
...
Если SDK робота не умеет читать battery voltage, сайт честно покажет
`нет данных`.
...
маленькой скорости и `engine_rev.wav`, когда скорость выше. Настройки лежат в
`config.yaml`:
