...
Она показывает live-состояние пульта, передний датчик расстояния, кнопки, стики,
последние события, логи и звуковую волну. Данные пульта, датчика и логов идут
по WebSocket `/ws/debug` с маленькой задержкой. Кнопка `Микрофон браузера`
рисует волну с микрофона устройства, на котором открыт сайт; отдельный
robot-mic backend можно добавить позже.
