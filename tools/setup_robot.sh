...
REPO_URL="https://github.com/knovikov/picar.git"
BRANCH="main"
PRIVATE_DEPLOY_KEY="0"
...
Options:
  ...
  --repo-url URL         Git clone URL. Default: https://github.com/knovikov/picar.git
  --branch NAME          Git branch to install. Default: main
  --private              Use a read-only deploy key for a private GitHub repo.
  --key PATH             Local deploy private key for --private mode.
                         Default: .deploy-keys/picar_robot_ed25519
  --reboot               Reboot Raspberry Pi after installation.
  --skip-github-key      In --private mode, do not add/check the deploy key with GitHub CLI.
  -h, --help             Show this help.

For public repos, the script clones over HTTPS and does not copy any GitHub key.
For private repos, pass --private to generate/copy a read-only deploy key.
EOF
}
...
ssh "$PI_HOST" \
  "ROBOT_DIR=$(shell_quote "$ROBOT_DIR") GIT_REMOTE_URL=$(shell_quote "$GIT_REMOTE_URL") BRANCH=$(shell_quote "$BRANCH") PRIVATE_DEPLOY_KEY=$(shell_quote "$PRIVATE_DEPLOY_KEY") GITHUB_HOST_ALIAS=$(shell_quote "$GITHUB_HOST_ALIAS") REMOTE_KEY_PATH=$(shell_quote "$REMOTE_KEY_PATH") REBOOT_AFTER=$(shell_quote "$REBOOT_AFTER") bash -s" <<'REMOTE_SETUP'
set -euo pipefail
